...
class _TDCalendarState extends State<TDCalendar> {
  ...
  _reGetTimePicker(time) {
    if (widget.timePickerModel != null && widget.timePickerModel!.isNotEmpty) {
      ...
      final currDateTimeEpoch = time[0];
      final currDateTime = DateTime.fromMillisecondsSinceEpoch(currDateTimeEpoch);
      List<int>? initDateStartList = widget.timePickerModel?[0]!.dateStart;
      final initDateStartEpoch = DateTime(initDateStartList![0], initDateStartList![1], initDateStartList![2],
              initDateStartList![3], initDateStartList![4], initDateStartList![5])
          .millisecondsSinceEpoch;

      useCurrDateTime(initDateStartEpoch, currDateTimeEpoch);
      ...
      datePickerModel = DatePickerModel(
          ...
          useMinute: true,
          useSecond: true,
          dateStart: currDateTimeEpoch <= initDateStartEpoch
              ? [
                  initDateStartList![0],
                  initDateStartList![1],
                  initDateStartList![2],
          ...
          dateInitial: initDateStartList![3] == preDateTime.hour &&
              ...
              : [currDateTime.year, currDateTime.month, currDateTime.day, 0, 0, 0]);
      // final newDateTime = DateTime(currDateTime.year, currDateTime.month, currDateTime.day, preDateTime.hour,
      //     preDateTime.minute, preDateTime.second)
      //     .millisecondsSinceEpoch;
      // selectedTime.value = [newDateTime];
    }
  }

  useCurrDateTime(initDateTime, selecDateTime) {
    if (selecDateTime > initDateTime) {
      final currDateTime = DateTime.fromMillisecondsSinceEpoch(selecDateTime);
      selectedTime.value = [selecDateTime];
      print(currDateTime);
    } else {
      final currDateTime = DateTime.fromMillisecondsSinceEpoch(initDateTime);
      selectedTime.value = [initDateTime];
      print(currDateTime);
    }
  }
          ...
          return Container(
            ...
            child: Row(
              children: List.generate(
                noRange ? 1 : 2,
                (index) {
                  final timePickerModel = datePickerModel ??
                      widget.timePickerModel?.getOrNull(index) ??
                      DatePickerModel(
                        useYear: false,
                        useMonth: false,
                        useDay: false,
